...
const navGroups = [
  {
    ...
    items: [
      ...
      { label: "Attendance", icon: Activity, active: true },
  ...
  },
  {
    ...
    items: [
      ...
      { label: "Automations", icon: Zap, badge: "NEW" },
    ],
  },
];
      ...
      <aside className={`sidebar ${open ? "sidebar-open" : ""}`}>
        ...
        <div className="sidebar-bottom">
          <div className="insight-card">
            ...
            <p>Your busiest window starts in 20 minutes.</p>
            <button>
...
function StatCard({
  ...
  return (
    <article className="attendance-stat-card">
      <div className={`attendance-stat-icon ${tone}`}>
        <Icon size={18} />
      </div>
      <div className="attendance-stat-label">
        {label}
        <button aria-label={`About ${label}`} title={detail}>
          <CircleHelp size={13} />
        </button>
      </div>
      <strong>{value}</strong>
      <p className={trend ? "stat-trend" : ""}>
...
function TrafficChart({ range }: { range: Range }) {
  ...
  return (
    <div className="traffic-chart" aria-label={`Attendance traffic for ${range}`}>
      <div className="traffic-y-axis">
        <span>{range === "Today" ? "75" : range === "7 days" ? "450" : "1.8k"}</span>
        <span>{range === "Today" ? "50" : range === "7 days" ? "300" : "1.2k"}</span>
        <span>{range === "Today" ? "25" : range === "7 days" ? "150" : "600"}</span>
        <span>0</span>
      </div>
      <div className="traffic-plot">
        ...
        <div className="traffic-bars">
          {hourlyTraffic.map((visits, index) => (
            <div className="traffic-column" key={hourLabels[index]}>
              <button
                className={`traffic-bar ${index === 12 ? "peak" : ""}`}
                style={{ height: `${Math.max(7, (visits / 72) * 100)}%` }}
                aria-label={`${hourLabels[index]}, ${Math.round(visits * multiplier)} check-ins`}
              >
                <span className="traffic-tooltip">
                  {Math.round(visits * multiplier)}
                  <small>check-ins</small>
                </span>
              </button>
              <span>{index % 2 === 0 || index === 15 ? hourLabels[index] : ""}</span>
            </div>
          ))}
        </div>
      </div>
      ...
      >
        <div className="modal-header">
          <div>
            ...
            <h2 id="checkin-title">Check in a member</h2>
            <p>Search for a member and record their arrival.</p>
          </div>
        ...
        <div className="recent-member">
          <span className="avatar blue">AV</span>
          <div>
            <strong>Ananya Verma</strong>
            <small>Annual Unlimited · Active</small>
          </div>
          <button type="button" onClick={() => setName("Ananya Verma")}>
            Select
          </button>
        </div>
        <label>
          ...
          <select defaultValue="Front desk">
            ...
            <option>QR kiosk</option>
            <option>Trainer entry</option>
          </select>
        </label>
...
export default function AttendancePage() {
  ...
  const filteredVisits = useMemo(
    () =>
      visits.filter(
        (visit) =>
          (filter === "All visits" || visit.status === filter) &&
          (!query.trim() ||
            `${visit.name} ${visit.plan} ${visit.source}`
              .toLowerCase()
  ...
  function checkOut(id: number) {
    setVisits((current) =>
      current.map((visit) =>
        visit.id === id ? { ...visit, status: "Checked out", checkOut: "8:30 PM" } : visit,
      ),
    );
    notify("Member checked out successfully");
  }
  function checkIn(name: string) {
    const memberName = name.trim() || "Ananya Verma";
    setVisits((current) => [
      {
        id: Date.now(),
        name: memberName,
  ...
  function exportVisits() {
    ...
    const csv = ["Member,Plan,Check in,Check out,Duration,Status,Source", ...rows].join("\n");
    const url = URL.createObjectURL(new Blob([csv], { type: "text/csv" }));
    const link = document.createElement("a");
    link.href = url;
    link.download = "pulse-fitness-attendance.csv";
    link.click();
  ...
  return (
    <div className="app-shell attendance-app">
      ...
      <div className="app-content">
        ...
        <main className="dashboard attendance-dashboard">
          <div className="attendance-heading">
            <div>
              ...
              <h1>Attendance</h1>
              <p>Track member visits and manage today’s check-ins.</p>
            </div>
            <div className="heading-actions">
              <div className="date-stepper">
                ...
                <div>
                  ...
                  <span>Today, 8 Aug</span>
                </div>
                <button aria-label="Next day" disabled>
                  <ChevronRight size={16} />
                </button>
              </div>
              <button className="button secondary export-button" onClick={exportVisits}>
                <Download size={16} /> Export
              </button>
              <button className="button primary" onClick={() => setShowModal(true)}>
                <Plus size={17} /> Check in member
              </button>
            </div>
          </div>
          <section className="attendance-stats" aria-label="Today’s attendance overview">
            <StatCard
              icon={UserRoundCheck}
              label="In gym now"
              value={`${84 + insideDelta}`}
              detail="12% above usual"
              trend="up"
              tone="green"
            />
            <StatCard
              ...
              value={`${386 + visits.length - initialVisits.length}`}
              detail="42 more than last Sat"
              trend="up"
            />
            <StatCard
              icon={Clock3}
            ...
            <StatCard
              ...
              label="Avg. visit"
              value="74 min"
              detail="4 min shorter this week"
              trend="down"
              tone="blue"
            />
          </section>
          <section className="attendance-overview-grid">
            <article className="panel traffic-panel">
              <div className="attendance-card-header">
                <div>
                  <h2>Check-in traffic</h2>
                  <p>Hourly member arrivals at South Delhi</p>
                </div>
                <div className="range-tabs">
                  {(["Today", "7 days", "30 days"] as Range[]).map((item) => (
                    <button
                      key={item}
              ...
              <div className="traffic-summary">
                <div>
                  <strong>
                    {range === "Today" ? "386" : range === "7 days" ? "2,238" : "9,341"}
                  </strong>
                  <span>check-ins</span>
                  <em>
            ...
            <article className="panel live-panel">
              <div className="attendance-card-header">
                <div>
                  <h2>Currently in gym</h2>
                  <p>{84 + insideDelta} active visits</p>
                </div>
              ...
              <div className="capacity-block">
                <div
                  ...
                  style={{ "--capacity": "67%" } as React.CSSProperties}
                >
                  <div>
                    <strong>67%</strong>
                    <span>capacity</span>
                ...
                <div className="capacity-copy">
                  <strong>{84 + insideDelta} of 125</strong>
                  <span>members inside</span>
                  <small>
                    <i /> Comfortable capacity
                  </small>
                </div>
              </div>
              <div className="live-member-list">
                {liveMembers.map((member) => (
                  <button key={member.name} onClick={() => notify(`${member.name} profile opened`)}>
              ...
              <button
                ...
                onClick={() => {
                  setFilter("In gym");
                  document.getElementById("visit-log")?.scrollIntoView({ behavior: "smooth" });
                }}
              >
                View everyone inside <ChevronRight size={15} />
              </button>
            </article>
          </section>
          <section className="panel visit-log" id="visit-log">
            <div className="visit-log-header">
              <div>
                <h2>Visit log</h2>
                <p>All check-ins recorded today</p>
              </div>
              <button className="icon-button small" aria-label="Visit log options">
                <MoreHorizontal size={18} />
              </button>
            </div>
            <div className="visit-toolbar">
              <div className="visit-tabs">
                {(["All visits", "In gym", "Checked out"] as VisitFilter[]).map((item) => (
                  <button
                    key={item}
                  ...
                  >
                    ...
                    <span>
                      {item === "All visits"
                        ? 386 + visits.length - initialVisits.length
                        : item === "In gym"
                          ? 84 + insideDelta
                          : 302 + checkoutDelta}
                    </span>
              ...
              <div className="visit-tools">
                ...
                <button
                  className={`button secondary filter-button ${showFilters ? "active" : ""}`}
              ...
              <div className="visit-advanced-filters">
                ...
                <label>
                  ...
                  <select defaultValue="All plans">
                    ...
                    <option>Annual Unlimited</option>
                    <option>Strength Pro</option>
                    <option>Monthly Flex</option>
                  </select>
                </label>
                <button onClick={() => setShowFilters(false)}>Clear filters</button>
              </div>
            )}
                ...
                <tbody>
                  {filteredVisits.map((visit) => (
                    <tr key={visit.id}>
                      <td>
                        <button
                          className="visit-member"
                          onClick={() => notify(`${visit.name} profile opened`)}
                        >
                          <span className={`avatar ${visit.tone}`}>{visit.initials}</span>
                          <span>
                            <strong>{visit.name}</strong>
                            <small>{visit.plan}</small>
                          </span>
                        </button>
                      </td>
                      <td>
                        <strong className="visit-time">{visit.checkIn}</strong>
                      </td>
                      <td>{visit.checkOut}</td>
                      <td>{visit.duration}</td>
                      <td>
                        <span
                          className={`visit-status ${visit.status === "In gym" ? "active" : "complete"}`}
                        >
                          <i />
                          {visit.status}
                        </span>
                      ...
                      <td>
                        {visit.status === "In gym" ? (
                          <button className="checkout-button" onClick={() => checkOut(visit.id)}>
                            <LogOut size={14} /> Check out
                          </button>
                        ) : (
                          <button
                            className="icon-button small"
                            aria-label={`More actions for ${visit.name}`}
                          >
            ...
            <div className="table-pagination">
              <span>
                Showing 1–{filteredVisits.length} of{" "}
                {filter === "All visits" ? 386 : filter === "In gym" ? 84 : 302} visits
              </span>
              <div>
                <button disabled>
                  <ChevronLeft size={15} />
          ...
          <footer className="dashboard-footer">
            <span>Attendance synced just now</span>
            <span>
      ...
      {showModal && <CheckInModal close={() => setShowModal(false)} checkIn={checkIn} />}
      {toast && (
